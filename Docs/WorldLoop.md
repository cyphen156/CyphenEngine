...
```
호출당 실행 비용        static_cast    dynamic_cast
구체 타입 바인딩            5.0ns         20.7ns
기반 클래스 바인딩         13.1ns         29.0ns
```

바인딩 효과는 `static_cast`에서 +8.1ns, `dynamic_cast`에서 +8.3ns입니다. 캐스팅 효과는 구체 타입에서 +15.7ns, 기반 클래스에서 +15.9ns입니다. 두 비용은 서로 영향을 주지 않고 가법으로 합산되므로, 두 결정은 독립적으로 평가할 수 있습니다.

최악 조건과 현재 구현의 차이는 호출당 24ns, 프레임당 0.36ms이며 실행 비용 기준 약 5.8배입니다.

### 번외 · Tick 구간 직접 계측

위 수치는 프레임 전체 시간의 차이에서 유도한 값이라 렌더링과 present 비용이 섞여 있습니다. 이를 분리하기 위해 `GameRuntime::Tick` 구간만 직접 계측했습니다. 측정 코드는 임시로 넣었다 제거했으며 커밋하지 않았습니다.

```
Release · Tick 구간만 · 표본 20000 프레임

실행 대상 없음        0.000153ms
Object 10000         0.109565ms   (호출당 7.30ns)
```

실행 대상이 없을 때 Tick의 고정 비용은 프레임당 **153ns**입니다. World 세 개 순회와 빈 그룹 조회를 포함한 값이며, 구조 자체에는 상시 비용이 사실상 없다는 것을 직접 계측으로 확인했습니다.

절대 비용을 인용할 때는 이 측정을, 조건 간 상대 비교에는 위 매트릭스를 사용합니다.
